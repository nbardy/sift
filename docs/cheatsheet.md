...
```
(rg "regex")                    ripgrep — exact, no index
(lex "query terms")             BM25 tantivy — ranked, indexed
(sem "natural language")        embedding similarity — conceptual
```
...
```
(& e ...)                       intersection — hits in ALL
(| e ...)                       union — hits in ANY
(mix e ...)                     blend via RRF (equal weight)
(mix [w1 w2 ...] e ...)        weighted blend
(- e1 e2)                      difference — e1 minus e2
(pipe e1 e2)                   sequential — run e1, scope e2 to those files
```

## Filters

```
(top k e)                       top k by score
...
```
(let [name expr ...] body)      bind intermediate results
```
...
;; find by concept
(sem "error handling and recovery")

;; blend exact + semantic
(mix (rg "TODO|FIXME") (sem "technical debt"))

;; multi-criteria intersection
(& (sem "authentication" :lang "rust") (sem "database" :lang "rust"))

;; weighted blend, top results
(top 10 (mix [0.6 0.4] (sem "retry backoff") (rg "retry|backoff")))

;; narrow then refine (sequential pipeline)
(pipe (rg "pub struct") (rg "impl"))

;; three-way fusion
(top 10 (mix (sem "entry point") (rg "fn main") (lex "main entry")))

;; reuse with let
(let [auth (rg "auth" :lang "rs")] (top 5 (- auth (rg "test"))))
```
